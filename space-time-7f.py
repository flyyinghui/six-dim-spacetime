...
class SixDimensionalSpacetimeHypergraph:
    ...
    def _generate_blackhole_positions(self, n_blackholes, scale=20.0):  # 扩大时空范围
        ...
        # 增加星系团中心之间的间距，扩大10倍
        cluster_centers = np.random.normal(0, 1, (n_clusters, 3)) * scale * 5.0  # 扩大10倍
        ...
        # 计算每个星系团的黑洞数量
        cluster_sizes = np.random.randint(min_bh_per_cluster, max_bh_per_cluster + 1, n_clusters)
        total_bh = sum(cluster_sizes)
        ...
        for i in range(n_clusters):
            ...
            cluster_size = min(cluster_sizes[i], n_blackholes - len(positions))
            
            # 在星系团内生成黑洞位置
            for _ in range(cluster_size):
                # 使用指数分布使黑洞更集中在中心
                # 保持星系团内部相对距离不变，只调整星系团之间的间距
                r = np.random.exponential(scale=0.3) * scale * 0.15  # 保持星系团内部密度不变
    ...
    def _generate_darkenergy_filaments(self, n_points, blackhole_positions, scale=20.0):  # 扩大时空范围
        ...
        if n_blackholes > 1:
            kmeans = KMeans(n_clusters=n_clusters, random_state=42).fit(blackhole_positions)
            cluster_centers = kmeans.cluster_centers_
            
            # 构建Delaunay三角剖分来连接聚类中心
    ...
    def _generate_earth_positions(self, n_points, blackhole_positions, darkenergy_positions, scale=20.0):  # 扩大时空范围
        ...
        if n_blackholes > 0:
            ...
            for _ in range(n_galaxy):
                # 随机选择一个黑洞
                bh_idx = np.random.randint(0, n_blackholes)
                center = blackhole_positions[bh_idx]
                
                # 使用指数分布生成径向距离，使点更集中在中心
                r = np.random.exponential(scale=0.5) * scale * 0.2  # 调整地球节点分布
                ...
                # 添加一些平面内的随机性（形成盘状结构）
                if random.random() < 0.7:  # 70%的概率在平面内
                    z *= 0.1  # 压扁z轴，形成盘状
